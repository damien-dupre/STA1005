...
The console displays 1. **What has been run**, 2. The **results** (or some parts) of what has been run, and 3. The **status of the R process**.
...
**.R** is the extension for an R script (document including only R code): 
...
* Video tutorials on YouTube, TikTok (@chelseaparlettpelleriti, @tommyteaches, ...)
...
However, this task can be long, difficult and lead to human errors. Thankfully, R has additional packages that provide alternative functions to read linear regression models and communicate results. Because there are too many packages, I will focus only on two additional packages: {performance} and {report}.
