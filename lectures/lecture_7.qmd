...
::: {style="font-size: 80%;"}
* Proprietary languages (e.g., Matlab, Stata, SAS) and software (e.g., SPSS) are outdated
...
1. **What has been run**, 
2. The **results** (or some parts) of what has been run, and 
3. The **status of the R process**.
...
* With the **Help** menu for R functions you can access essential information to use them. Just have a look at some of them by pressing <kbd>F1</kbd> with the cursor on the function, using the `help()` function, or by typing `?` followed by the function name such as:
...
- Includes code which can be active or inactive (after `#`)
...
* The second argument `to` is the last number of the sequence
...
```{r}
include_graphics("https://bookdown.org/introrbook/intro2r/images/chapter5/fig1.PNG")
```
...
1. Click on `Upload` to upload the "organisation_beta.csv" file on your Posit Cloud

2. Click on `Import` to import these data in R

3. Check that the data appears in the Environment pane. How many observations and variables does it have?

4. Copy the code that ran and paste it in your R script
...
* Book tutorials, see for example:
  ...
  - [A ModernDive into R and the tidyverse](https://moderndive.com/v2/) by Ismay & Kim (2025)
  - [Open Data Science](https://ohi-science.org/data-science-training/) by the Ocean Health Index Team (2019)
  
Find all the free books in the [Big Book of R](https://www.bigbookofr.com/)!
...
- One or more Predictor Variables of any type (categorical or continuous)
...
## Automatic Results with {report} {.smaller}
